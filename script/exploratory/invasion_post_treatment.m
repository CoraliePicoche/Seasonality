...
s1=17;
s2=55; %it will eventually be 1->60, keeping in mind the fact that SV only observe stuff for 14->43 
    ...
    S=size(youtbis,2); %number of species
        ...
        if(sum(mask)>1)
            ...
            if sum(mask)==4
                ...
                id1tmp=min(idtmp);
                id2tmp=max(idtmp);
                 mat(id1tmp-s1+1,id2tmp-s1+1)=mat(id1tmp-s1+1,id2tmp-s1+1)+1;
                mat_col(id1tmp-s1+1,id2tmp-s1+1)=idtmp(idtmp>id1tmp&idtmp<id2tmp)-s1+1;
                idtmp=id(id>id1);
                id1tmp=min(idtmp);
                id2tmp=max(idtmp);
                 mat(id1tmp-s1+1,id2tmp-s1+1)=mat(id1tmp-s1+1,id2tmp-s1+1)+1;
...
for i=1:length(xx)
    for j=1:length(yy)
        if mat(i,j)>0 
            if extant==2 
                plot(xx(i),yy(j),'o','MarkerFaceColor',[grey_scale(mat(i,j)) grey_scale(mat(i,j)) grey_scale(mat(i,j))],'color',[grey_scale(mat(i,j)) grey_scale(mat(i,j)) grey_scale(mat(i,j))])   
            else
                ...
                plot(xx,repelem(20.42,length(xx)),'-k')
                plot(repelem(20.42,length(xx)),yy,'-k')
hold off;
