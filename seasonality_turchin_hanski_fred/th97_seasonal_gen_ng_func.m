...
Winter = (1+cos(2*pi*t)); 
% tmod = mod(t,1.0); % For threshold version of seasonality
% if (tmod>0.25)&&(tmod<0.75), Winter = 0; else Winter = 2; end;
...
dydt = zeros(3,1);   
M1 = r*y(1);
M2 = g*y(1)*(2-Winter)/(y(1)^2+h^2); % g*(1-Winter)*y(1)^2/(y(1)^2+h^2)% check more abrupt functional response instead -> change the effect of seasonality?
M3 = a*y(2)/(y(1)+d);
dydt(1) = (r - M1 - M2 - M3)*y(1);  % Now 2-Winter as the max of this variable is 2 now (mean=1), so that 1-cos is in the parenthesis
